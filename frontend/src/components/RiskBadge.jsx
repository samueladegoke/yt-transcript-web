...
export function RiskBadge({ clean, failed }) {
  if (failed) {
    return (
      <motion.div
        initial={{ opacity: 0, scale: 0.9 }}
        {...badgeAnimation} role="status" aria-label="Risk status: Failed" className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-red-500/10 border border-red-500/20"
      >
  ...
  if (clean) {
    return (
      <motion.div
        initial={{ opacity: 0, scale: 0.9 }}
        {...badgeAnimation} role="status" aria-label="Risk status: Clean" className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-[#C8A941]/10 border border-[#C8A941]/20"
      >
  ...
  return (
    <motion.div
      initial={{ opacity: 0, scale: 0.9 }}
      {...badgeAnimation} role="status" aria-label="Risk status: Risky" className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-[#00D4FF]/10 border border-[#00D4FF]/20"
    >
